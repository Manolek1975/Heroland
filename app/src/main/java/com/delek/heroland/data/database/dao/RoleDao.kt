...
interface RoleDao {
    ...
    @Query("SELECT dwellings.* FROM dwellings INNER JOIN role_dwellings " +
    ...
    @Query("SELECT roles.* FROM roles LEFT JOIN players " +
            "ON roles.id = players.role " +
